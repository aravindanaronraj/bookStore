...
const Register = () => {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); if (form.password !== form.confirmPassword) { setError("கடவுச்சொற்கள் பொருந்தவில்லை"); return; } try { setLoading(true); setError(""); const result = await registerUser({ name: form.name, email: form.email, phone: form.phone, password: form.password }); setVerification(true); setMessage(result.message || "மின்னஞ்சலுக்கு அனுப்பப்பட்ட 6 இலக்கக் குறியீட்டை உள்ளிடுங்கள்."); } catch (e: unknown) { setError(getErrorMessage(e, "பதிவு செய்ய முடியவில்லை")); } finally { setLoading(false); } };
