...
exports.manageBookRemoval = exports.deleteBook = exports.updateBook = exports.getBookById = exports.getBooks = exports.createBook = void 0;
const mongoose_1 = __importDefault(require("mongoose"));
const Book_1 = __importDefault(require("../models/Book"));
const cloudinary_1 = __importDefault(require("../config/cloudinary"));
const Cart_1 = __importDefault(require("../models/Cart"));
const Review_1 = __importDefault(require("../models/Review"));
const asBoolean = (value, fallback = false) => value === undefined ? fallback : value === true || value === "true";
// CREATE BOOK
const createBook = async (req, res) => {
    try {
        const { title, slug, author, publisher, isbn, description, category, images, bookType, price, salePrice, stock, language, pages, isFeatured, isNewLaunch, isActive, } = req.body;
        if (!title ||
            !slug ||
            !author ||
            !description ||
            ...
            res.status(400).json({
                success: false,
                message: "Title, slug, author, description, category, price and language are required",
            });
            return;
        }
...
const getBooks = async (req, res) => {
    try {
        const { search, category, language, bookType, featured, newLaunch, minPrice, maxPrice, page = "1", limit = "12", } = req.query;
        ...
        const [books, total] = await Promise.all([
            Book_1.default.find(filter)
                .populate("category", "name slug")
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limitNumber),
            Book_1.default.countDocuments(filter),
        ]);
...
const getBookById = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id || Array.isArray(id) || !mongoose_1.default.Types.ObjectId.isValid(id)) {
            res.status(400).json({
                success: false,
...
const updateBook = async (req, res) => {
    try {
        ...
        const { title, slug, author, publisher, isbn, description, category, images, bookType, price, salePrice, stock, language, pages, isActive, isFeatured, isNewLaunch, } = req.body;
        if (title !== undefined)
        ...
        if (publisher !== undefined)
            book.publisher = typeof publisher === "string" && publisher.trim() ? publisher.trim() : undefined;
        if (isbn !== undefined)
            book.isbn = typeof isbn === "string" && isbn.trim() ? isbn.trim().toUpperCase() : undefined;
        ...
        if (salePrice !== undefined) {
            if (salePrice === "" || salePrice === null)
                ...
                if (!Number.isFinite(parsedSalePrice) || parsedSalePrice <= 0 || parsedSalePrice >= regularPrice) {
        ...
        if (isActive !== undefined)
            book.isActive = asBoolean(isActive);
        if (isFeatured !== undefined) {
            book.isFeatured = asBoolean(isFeatured);
        }
        if (isNewLaunch !== undefined)
            book.isNewLaunch = asBoolean(isNewLaunch);
        await book.save();
        const populatedBook = await book.populate("category");
        res.status(200).json({
            success: true,
...
const manageBookRemoval = async (req, res) => {
    try {
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const action = req.body?.action;
        if (!id || !mongoose_1.default.Types.ObjectId.isValid(id) || !["hide", "delete"].includes(action)) {
            res.status(400).json({ success: false, message: "Invalid removal request" });
            return;
        }
        const book = await Book_1.default.findById(id);
        if (!book) {
        ...
        if (action === "hide") {
            book.isActive = false;
            await book.save();
            res.json({ success: true, message: "Book hidden from store" });
            return;
        }
        const imageIds = book.images.map((image) => image.publicId).filter(Boolean);
        if (imageIds.length)
            await cloudinary_1.default.api.delete_resources(imageIds).catch((error) => console.error("Book image cleanup failed:", error));
        await Cart_1.default.updateMany({ "cartItems.book": book._id }, { $pull: { cartItems: { book: book._id } } });
        await Review_1.default.deleteMany({ book: book._id });
        await book.deleteOne();
        res.json({ success: true, message: "Book permanently deleted" });
    }
    catch (error) {
        console.error("Book removal error:", error);
        res.status(500).json({ success: false, message: "Unable to remove book" });
    }
};
exports.manageBookRemoval = manageBookRemoval;
