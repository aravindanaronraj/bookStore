...
exports.getMyOrderById = exports.getMyOrders = exports.verifyPayment = exports.createOrder = void 0;
...
const createOrder = async (req, res) => {
    try {
        ...
        const orderItems = cart.cartItems.map((item) => {
            ...
            if (!book)
                throw new Error("Your cart contains an unavailable book");
            ...
            const price = book.salePrice && book.salePrice > 0
                ? book.salePrice
                : book.price;
        ...
        if (couponCode) {
            ...
            const userUsageCount = await CouponUsage_1.default.countDocuments({
                coupon: coupon._id,
                user: req.user.id,
            });
...
const verifyPayment = async (req, res) => {
    try {
        ...
        const signatureMatches = expectedSignature.length === receivedSignature.length &&
            crypto_1.default.timingSafeEqual(expectedSignature, receivedSignature);
        if (!signatureMatches) {
        ...
        const claimedOrder = await Order_1.default.findOneAndUpdate({ _id: order._id, paymentStatus: { $in: ["pending", "failed"] } }, { $set: { paymentStatus: "processing", razorpayPaymentId: razorpay_payment_id } }, { returnDocument: "after" });
        if (!claimedOrder) {
            const currentOrder = await Order_1.default.findById(order._id);
        ...
        try {
            for (const item of claimedOrder.items) {
                const book = await Book_1.default.findById(item.book);
                if (!book)
                    throw new Error(`Book not found: ${item.title}`);
                if (book.bookType === "ebook")
                    continue;
                const stockUpdate = await Book_1.default.updateOne({ _id: book._id, stock: { $gte: item.quantity } }, { $inc: { stock: -item.quantity } });
            ...
            if (claimedOrder.coupon) {
                await CouponUsage_1.default.create({ coupon: claimedOrder.coupon, user: req.user.id, order: claimedOrder._id });
                couponUsageCreated = true;
                await Coupon_1.default.findByIdAndUpdate(claimedOrder.coupon, { $inc: { usedCount: 1 } });
            ...
            await Cart_1.default.findOneAndUpdate({ user: req.user.id }, { $set: { cartItems: [] } });
        }
        catch (finalizationError) {
            await Promise.all(deductedItems.map((item) => Book_1.default.updateOne({ _id: item.bookId }, { $inc: { stock: item.quantity } })));
            if (couponUsageCreated)
                await CouponUsage_1.default.deleteOne({ order: claimedOrder._id });
            if (couponCountIncremented && claimedOrder.coupon) {
                await Coupon_1.default.findByIdAndUpdate(claimedOrder.coupon, { $inc: { usedCount: -1 } });
            }
            let failedPaymentStatus = "failed";
            try {
                await razorpay_1.default.payments.refund(razorpay_payment_id, {
            ...
            await Order_1.default.findByIdAndUpdate(claimedOrder._id, {
